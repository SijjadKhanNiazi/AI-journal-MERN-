...
const generatequize = async (req, res) => {
  try {
    const note = await Note.findById(req.params.id);
    if (!note) return res.status(404).json({ message: "Note not found" });

    const FASTAPI_URL = "http://127.0.0.1:8000/generate-quiz";

    const aiResponse = await axios.post(FASTAPI_URL, {
      text: note.content,
    });

    note.quiz = aiResponse.data.quiz;
    await note.save();

    res.json(note);
  } catch (error) {
    console.error("AI Error:", error.message);
    res.status(500).json({ message: "AI Quiz generator failed" });
  }
};
